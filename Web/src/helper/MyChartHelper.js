...
export class MyChartHelper extends Component {
  ...
  static sortCountedData(countedData) {
  ...
  static getColorByColoringMode(coloringMode,min,max,value,index,amount){
    ...
    if(coloringMode===MyChartHelper.coloringModeHigherIsBetter){
        return ColorHelper.interpolateMinMaxValue(green,red,min,max,value);
    }
    if(coloringMode===MyChartHelper.coloringModeLowerIsBetter){
      return ColorHelper.interpolateMinMaxValue(red,green,min,max,value);
    }
    if(coloringMode===MyChartHelper.coloringModeFirstIsBetter){
      return ColorHelper.interpolateMinMaxValue(red,green,min,amount-1,index);
    }
    if(coloringMode===MyChartHelper.coloringModeLastIsBetter){
      return ColorHelper.interpolateMinMaxValue(green,red,min,amount-1,index);
    }
  ...
  static getDatasetForCountedData(
    ...
    if (sorting) {
      sortedMostCountKey = MyChartHelper.sortCountedData(countedData);
    }
