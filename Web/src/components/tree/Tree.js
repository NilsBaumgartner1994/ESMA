...
export class Tree extends Component {
    ...
    onDropPoint(event) {
        if (this.validateDropPoint(event)) {
            ...
            if (event.position < 0) {
                let dropIndex = (siblings) ? (this.dragState.index > event.handleIndex) ? event.handleIndex : event.handleIndex - 1 : event.handleIndex;
    ...
    validateDropPoint(event) {
        ...
        if (validateDrop) {
            ...
            if (event.position === -1 && this.areSiblings(this.dragState.path, event.path) && (this.dragState.index +1 === event.handleIndex)) {
                return false;
            }
